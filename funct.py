...
def input_line(connection):
     tline=b''
     while True:
        data = connection.recv(256) # input from user
        if not data:
...
#similar to commodore basic get command (waits for one character from user)
def get_char(connection):
     while True:
        data = connection.recv(256) #receives one character from user
        if not data:
           print("no data - closed connection")
           connection.close()
           break
...
# send a SEQ file to the user (useful for petscii graphics)
